...
{
    fct_1 fonction = cli->data_lib[13].fct;
    return fonction(array[1]);
}

int unknown_channel_function(client *cli, char **array)
{
    fct_1 fonction = cli->data_lib[14].fct;
    return fonction(array[1]);
}

int unknown_thread_function (client *cli, char **array)
{
    fct_1 fonction = cli->data_lib[15].fct;
    return fonction(array[1]);
}

int unknown_user_function   (client *cli, char **array)
{
    fct_1 fonction = cli->data_lib[16].fct;
    return fonction(array[1]);
}

int unauthorized_function   (client *cli, char **array)
{
    fct_0 fonction = cli->data_lib[17].fct;
    return fonction();
}
